...
// Initialize AI later when actually needed to prevent top-level crashes if API key is missing in local/vercel envs
const getAI = () => {
...
export function Chatbot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: '1', role: 'model', content: "Hi! I'm Ahmad's personal AI assistant. What would you like to know about him?" }
  ]);
    ...
    try {
      ...
      try {
        ...
        setMessages(prev => [...prev, { 
          ...
          role: 'model', 
          content: "Oops! Looks like the GEMINI_API_KEY environment variable is missing on this deployment! [SHOW_WHATSAPP_BUTTON]" 
        }]);
        setIsTyping(false);
        return;
      }

      // Build conversation history format needed for generateContent
      const history = messages.slice(1).map(m => ({ // skip initial greeting for history mapping if needed, or include it
        role: m.role,
      ...
      const response = await aiClient.models.generateContent({
        model: 'gemini-3.1-flash-lite-preview', // Quickest model for chat
        contents: history,
        config: {
      ...
      setMessages(prev => [...prev, { 
        id: (Date.now() + 1).toString(), 
        role: 'model', 
        content: "Oops! I encountered an error connecting to my brain. Please email Ahmad directly!" 
      }]);
    } finally {
      setIsTyping(false);
    }
          ...
          >
            <div className="flex items-center justify-between p-4 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-sm text-zinc-900 dark:text-white">Ahmad's AI Bot</h3>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.map((msg) => {
                const hasWhatsApp = msg.content.includes('[SHOW_WHATSAPP_BUTTON]');
                const displayText = msg.content.replace('[SHOW_WHATSAPP_BUTTON]', '').trim();
              ...
              {isTyping && (
                <div className="flex gap-3 justify-start">
                  ...
                  <div className="bg-zinc-100 dark:bg-zinc-800 rounded-2xl rounded-tl-sm px-4 py-3 flex items-center gap-1">
                    <motion.div animate={{ y: [0, -3, 0] }} transition={{ repeat: Infinity, duration: 0.6 }} className="w-1.5 h-1.5 bg-zinc-400 rounded-full" />
            ...
            <form onSubmit={handleSubmit} className="p-3 border-t border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-900">
